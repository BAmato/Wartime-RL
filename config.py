...
class RewardConfig:
    ...
    deploy: float = +0.1 #0.05          # per army placed during reinforce phase
    pass_attack: float = -1.0 #-0.3  # was -0.5 — much stronger penalty

    border_pressure: float = -0.01   # penalty per turn based on enemy army ratio at borders
    defensive_fortify: float = +1.0  # reward for fortifying a territory adjacent to an enemy

    survival: float = -0.3
    continent_scale: float = +2.0

    win_game: float = +250.0
...
class CurriculumTracker:
    ...
    def win_rate(self) -> float:
        ...
        if not self._history:
            return 0.0
        window_size=self.cfg.eval_window
        recent_history=self._history[-window_size:]
        return self._history.count("win") / len(recent_history)
